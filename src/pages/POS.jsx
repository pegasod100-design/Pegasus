...
const css = `
  ...
  .pos-prod-row.sin-stock { opacity:.5; cursor:not-allowed; background:#fef2f2; }
  ...
  .pos-cobrar { padding:14px; background:#16a34a; color:#fff; border:none; border-radius:8px; font-size:16px; font-weight:800; cursor:pointer; width:100%; }
  .pos-cobrar:hover:not(:disabled) { background:#15803d; }
  .pos-cobrar:disabled { opacity:.6; cursor:not-allowed; }
  ...
  .email-section { background:#f0fdf4; border:1.5px solid #bbf7d0; border-radius:10px; padding:12px 14px; display:flex; flex-direction:column; gap:8px; }
  .email-input { width:100%; padding:9px 12px; border-radius:7px; border:1.5px solid #e2e8f0; font-size:13px; box-sizing:border-box; outline:none; transition:border-color .2s; }
...
export default function POS() {
  ...
  const handleSearchKeyDown = (e) => {
    ...
    if (filtrados.length === 0 && search.length > 1) {
      setAlertaStock({ nombre: search, noEncontrado: true });
      setTimeout(() => setAlertaStock(null), 3500);
      return;
    }
    if (filtrados.length === 1) intentarAgregar(filtrados[0]);
  };

  const intentarAgregar = (p) => {
    setAlertaStock(null);
    if (p.stock_actual !== undefined && p.stock_actual <= 0 && idTienda) {
      setAlertaStock({ nombre: p.nombre_producto, sinStock: true });
      setTimeout(() => setAlertaStock(null), 3500);
      setSearch('');
      return;
    }
    setCarrito(prev => {
      ...
      if (exist) return prev.map(i => i.codigo_producto === p.codigo_producto ? { ...i, cantidad: i.cantidad + 1 } : i);
  ...
  const cambiarCantidad = (codigo, delta) =>
    setCarrito(prev => prev.map(i => i.codigo_producto === codigo ? { ...i, cantidad: Math.max(1, i.cantidad + delta) } : i));
  ...
  const cobrar = async () => {
    ...
    try {
      const { data } = await createVenta({
        ...
        items: carrito.map(i => ({ codigo_producto: i.codigo_producto, cantidad_venta: i.cantidad, precio_unitario: i.precio_unitario }))
      });
      ...
      <div className="pos-root">
        ...
        <div className="pos-left" style={{ display:'flex', flexDirection:'column', gap:10 }}>
          ...
          {idTienda && !esAdmin
            ...
            : <select className="pos-select" value={idTienda} onChange={e => { setIdTienda(e.target.value); const t = tiendas.find(t => String(t.id_tienda) === e.target.value); setTiendaNombre(t?.nombre_tienda || ''); }}>
                <option value="">— Seleccionar tienda —</option>
                {tiendas.map(t => <option key={t.id_tienda} value={t.id_tienda}>{t.nombre_tienda}</option>)}
              </select>
          }

          <input ref={searchRef} className={`pos-search${alertaStock ? ' error' : ''}`}
            placeholder="🔍 Buscar o escanear código de producto..."
            value={search} onChange={e => { setSearch(e.target.value); setAlertaStock(null); }}
              ...
              <div>
                ...
                <div style={{ fontSize:12, color:'#ef4444', marginTop:2 }}>
                  {alertaStock.noEncontrado
                    ...
                    : `"${alertaStock.nombre}" no tiene stock en esta tienda.`}
                </div>
              </div>
            </div>
          )}
          ...
          {filtrados.length > 0 && !alertaStock && (
            <div className="pos-resultados">
              {filtrados.map(p => {
                ...
                return (
                  <div key={p.codigo_producto} className={`pos-prod-row${sinStock ? ' sin-stock' : ''}`}
          ...
          <div style={{ fontWeight:700, fontSize:13, color:'#64748b' }}>Artículos en carrito ({carrito.length})</div>
        ...
        <div className="pos-right">
          {!ticket ? (
            <div className="pos-resumen">
              ...
              {/* ── EMAIL CLIENTE — aquí en el resumen ── */}
              <div className="email-section">
                <div style={{ fontSize:12, fontWeight:700, color:'#15803d' }}>📧 Ticket al cliente (opcional)</div>
                <input className="email-input" placeholder="Nombre del cliente"
                  value={nombreCliente} onChange={e => setNombreCliente(e.target.value)} />
                <input className="email-input" type="email" placeholder="correo@cliente.com"
                  value={correoCliente} onChange={e => setCorreoCliente(e.target.value)} />
                {correoCliente && (
                  <div style={{ fontSize:11, color:'#16a34a', fontWeight:600 }}>✓ Ticket se enviará a {correoCliente}</div>
                )}
              </div>
            ...
            <div className="ticket-wrap">
              <div className="ticket-header">
                ...
                <div style={{ fontSize:12, opacity:.85, marginTop:2 }}>{new Date().toLocaleString('es-MX')}</div>
              </div>
              <div className="ticket-body">
                <div style={{ background:'#f8fafc', borderRadius:8, padding:'10px 12px', marginBottom:6 }}>
                  ...
                  <div style={{ fontSize:12, color:'#64748b', marginTop:2 }}>Cajero: <strong>{ticket.cajero}</strong></div>
                </div>
                <div style={{ fontWeight:700, fontSize:11, color:'#94a3b8', textTransform:'uppercase', letterSpacing:1, margin:'4px 0' }}>Productos</div>
                {ticket.items?.map(i => (
                ...
                <div style={{ marginTop:8 }}>
                  <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, color:'#64748b', padding:'3px 0' }}><span>Subtotal</span><span>${ticket.subtotal.toFixed(2)}</span></div>
                  <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, color:'#64748b', padding:'3px 0' }}><span>IVA (16%)</span><span>${ticket.iva.toFixed(2)}</span></div>
                  <div className="ticket-total"><span>TOTAL</span><span>${ticket.total.toFixed(2)}</span></div>
                </div>
                <div style={{ marginTop:8, display:'flex', flexDirection:'column', gap:4 }}>
                  {ticket.ticket_admin && <div style={{ fontSize:11, color:'#7c3aed', background:'#f5f3ff', padding:'5px 10px', borderRadius:6 }}>📧 Ticket enviado al administrador</div>}
                  {ticket.ticket_cliente && <div style={{ fontSize:11, color:'#16a34a', background:'#f0fdf4', padding:'5px 10px', borderRadius:6 }}>📧 Ticket enviado al cliente</div>}
                </div>
                <button style={{ marginTop:12, padding:'11px', background:'#2563eb', color:'#fff', border:'none', borderRadius:8, cursor:'pointer', fontWeight:700, width:'100%', fontSize:14 }}
                  onClick={() => { setTicket(null); searchRef.current?.focus(); }}>
                  + Nueva venta
                </button>
              </div>
            </div>
