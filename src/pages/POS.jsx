...
const css = `
  ...
  .pos-search { width:100%; padding:12px 16px; border-radius:8px; border:2px solid #2563eb; font-size:15px; outline:none; box-sizing:border-box; }
  ...
  .pos-prod-row { display:flex; justify-content:space-between; align-items:center; padding:10px 14px; cursor:pointer; border-bottom:1px solid #f1f5f9; }
  ...
  .pos-ticket { background:#fff; border-radius:12px; padding:16px; box-shadow:0 1px 8px rgba(0,0,0,.08); font-size:13px; display:flex; flex-direction:column; gap:6px; }
...
export default function POS() {
  ...
  useEffect(() => {
    ...
    // Si el empleado tiene id_tienda asignada, la usamos directamente
    const idTiendaEmpleado = empleado.id_tienda;
    if (idTiendaEmpleado) {
      const tienda = tiendas.find(t => String(t.id_tienda) === String(idTiendaEmpleado));
      if (tienda) {
        setIdTienda(String(tienda.id_tienda));
        setTiendaNombre(tienda.nombre_tienda);
        return;
      }
    }

    // Si la tienda viene en el objeto anidado (tiendas.nombre_tienda)
    const nombreTiendaEmpleado = empleado.tiendas?.nombre_tienda;
    if (nombreTiendaEmpleado) {
      const tienda = tiendas.find(t => t.nombre_tienda === nombreTiendaEmpleado);
      if (tienda) {
        setIdTienda(String(tienda.id_tienda));
        setTiendaNombre(tienda.nombre_tienda);
      }
    ...
    <>
      ...
      <div className="pos-root">
        ...
        <div className="pos-left" style={{ display:'flex', flexDirection:'column', gap:10 }}>
          ...
          {/* Tienda: automática para cajero, seleccionable solo para admin */}
          {idTienda && !esAdmin ? (
            <div className="tienda-badge">
              🏪 Tienda asignada: <strong>{tiendaNombre}</strong>
            </div>
          ) : (
          ...
          {!idTienda && !esAdmin && (
            <div style={{ background:'#fff7ed', border:'1px solid #fed7aa', borderRadius:8, padding:'10px 14px', fontSize:13, color:'#c2410c' }}>
              ⚠️ Tu usuario no tiene una tienda asignada. Contacta al administrador.
            </div>
          )}
        ...
        <div className="pos-right">
          ...
          {ticket && (
            <div className="pos-ticket">
              ...
              {ticket.ticket_enviado && (
                <div style={{ color:'#7c3aed', fontSize:11 }}>📧 Ticket enviado al administrador</div>
              )}
